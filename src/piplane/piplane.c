...
/**
 * Subscription callback for messages
 */
...
//     "/motor_array"); rclc_executor_init(&executor, &support.context, 1,
//     &allocator);
...
//     // Initialize controller 0 with DSHOT 300 on pio0, state machine 0, 4
//     channels starting on pin 2 int time;
...
// 		/* at the beginning of the first second, spin the first motor.
// On the second second, spin the second motor, etc */ 		for (int i = 0;
// i < 4; i++) { 			if (time > 2000)
// dshot_throttle(&controller0, i, motor_throttles[i]);
// else 				dshot_throttle(&controller0, i, 0);
// 		}
